function h = qqplotModif(x,y,pvec,num)
...
%   $Revision: 1.1.8.2 $  $Date: 2010/08/07 07:37:46 $
...
elseif nargin>=2 && isa(y,'ProbDist')
   ...
   xx = x;
   yy = y;
   xlab = sprintf('Quantiles of %s Distribution', pd.DistName);
   ylab = 'Quantiles of Input Sample';
   tlab = 'QQ Plot of Sample Data versus Distribution';
else
    ...
    tlab=num;
    xlab='Observed rain (mm)';
    ylab='Generated rain (mm)';
...
hrefmid = line(qx,qy,'LineStyle','none','Marker','none');
hdat = line(xx,yy,'LineStyle','-','color',[0.6 0.6 0.6],'LineWidth',2,'Marker','o','MarkerSize',10,'MarkerFaceColor',[1 1 1]);
if length(hdat)==1
    set(hdat,'MarkerEdgeColor','k');
    set([hrefends,hrefmid],'Color',[0.6 0.6 0.6]);
end
if nargout>0
...
title (tlab,'Interpreter','latex','FontSize',14);
...
if ~isempty(group)
    datatipTxt{end+1} = sprintf('Group: %s',num2str(group));
end
if ~isempty(origindx)
    dat = origindx(ind);
    if dat~=0
        datatipTxt{end+1} = sprintf('X observation: %s',num2str(dat));
    end
end
if ~isempty(origindy)
    dat = origindy(ind);
    if dat~=0
        datatipTxt{end+1} = sprintf('Y observation: %s',num2str(dat));
    end
end
