function S = variogram(x,y,scale,varargin)
...
%
%   scale - added by Luis - gives the resolution of the grid.
...
% Here is where the square difference of the rainfall values will be
% computed for two points separated certain distance.
...
switch params.type
    case {'default','gamma'}
        ...
        if params.anisotropy
            ...
            S.val      = accumarray(ixedge,lam,[numel(edges) 1],fvar,[]);     
            S.num      = accumarray(ixedge,ones(size(lam)),[numel(edges) 1],@sum,[]);
        end
