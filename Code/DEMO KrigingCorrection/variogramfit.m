...
if params.plotit
    switch lower(type)
        case 'bounded'
            plot(h,gammaexp,'r*','MarkerSize',6);
            hold on
            fplot(@(h) funnugget(b) + func(b,h),[0 b(1)],'--b')
            fplot(@(h) funnugget(b) + b(2),[b(1) max(h)],'--b')
            
        case 'unbounded'
            plot(h,gammaexp,'r*','MarkerSize',6);
            hold on
            fplot(@(h) funnugget(b) + func(b,h),[0 max(h)],'--b')
    end
    axis([0 max(h) 0 max(gammaexp)])
    xlabel('lag distance $h$','Interpreter','latex','FontSize',14)
    ylabel('$\gamma(h)$','Interpreter','latex','FontSize',14)
    hold off
end
