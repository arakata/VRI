...
%   $Revision: 1.1.8.3 $  $Date: 2010/08/07 07:37:42 $
...
hdat = line(sx,y,'LineStyle','none','Marker','*','markersize',3,'Color',[.6 .6 .6]);
if m==1
    set(hdat,'MarkerEdgeColor',[.6 .6 .6]);
...
set(gca,'YLim',[-3.5 4],'XLim',[minxaxis maxxaxis]);
xlabel('Rainfall','interpreter','latex','FontSize',14);
ylabel('Probability','interpreter','latex','FontSize',14);
title('Normal Probability Plot','interpreter','latex','FontSize',14);
...
datatipTxt = {
    sprintf('Data: %s',num2str(x)),...
    sprintf('Probability: %s',num2str(yper)),...
    ''
    };
datatipTxt{end+1} = sprintf('Observation: %s',num2str(origind));
if ~isempty(group)
    datatipTxt{end+1} = sprintf('Group: %s',num2str(group));
end

datatipTxt{end+1} = '';
datatipTxt{end+1} = sprintf('Refline Data: %s',num2str(xexp));
datatipTxt{end+1} = sprintf('Refline Probability: %s',num2str(yperexp));
